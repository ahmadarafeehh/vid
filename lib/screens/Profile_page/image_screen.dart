...
class _ImageViewScreenState extends State<ImageViewScreen> {
  ...
  // Rating state variables - updated to match PostCard
  double _averageRating = 0.0;
  int _totalRatingsCount = 0;
  double? _userRating;
  bool _isLoadingRatings = true;
  bool _isRating = false;
  late RealtimeChannel _postChannel; // Add realtime channel
  ...
  // Local ratings list to match PostCard structure
  ...
  void initState() {
    ...
    // Initialize local ratings list
    _localRatings = [];

    _fetchCommentsCount();
    _checkBlockStatus();
    _setupRealtime(); // Setup realtime updates
    _fetchInitialRatings(); // Fetch initial ratings
    _loadBannerAd();
  ...
  // Setup realtime channel for ratings updates
  void _setupRealtime() {
  ...
  // Handle realtime rating updates - same as PostCard
  ...
  // Update average rating from local ratings - same as PostCard
  ...
  // Fetch initial ratings - same logic as PostCard
  ...
  // Rating submission handler - same as PostCard
  void _handleRatingSubmitted(double rating) async {
    ...
    setState(() {
      ...
      // Optimistic update - same logic as PostCard
  ...
  void dispose() {
    ...
    _bannerAd?.dispose();
    _postChannel.unsubscribe(); // Unsubscribe from realtime updates
    super.dispose();
  }
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // Use the same RatingBar widget with updated props
                  RatingBar(
